...
Adapter 仅负责格式转换（JSON → Signal、round → DecisionRequest）；EvidenceProvider 负责语义归一化（signal → GovernanceEvidence，如 risk_level、verifiability）；Matrix 根据 evidence 执行治理策略（decision）。Adapter 是「搬运层」，EvidenceProvider 是「语义归一层」，Matrix 是「决策层」。
...
R0–R3 表示**操作副作用等级（Operational Side-effect Level）**，不是 domain 语义。各 domain 的 EvidenceProvider 必须按此标准映射。
