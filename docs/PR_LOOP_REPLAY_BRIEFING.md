...
**Interpretation：** case_001 展示真实 PR loop 治理路径；
case_002 用于隔离验证 loop-aware routing；
Permission domain 验证 scope_request → risk_level → decision 的跨 domain 接入能力。

These results demonstrate correctness of the governance logic on the current replay dataset, rather than statistical model evaluation.
